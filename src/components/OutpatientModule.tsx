...
import { Search, UserCheck, ChevronRight, Lock, Calendar } from 'lucide-react';
import { Patient } from '../types';
import { motion } from 'motion/react';
import { cn } from '../lib/utils';

interface OutpatientModuleProps {
  patients: Patient[];
  onOpenPatient: (id: string) => void;
}

export const OutpatientModule: React.FC<OutpatientModuleProps> = ({ patients, onOpenPatient }) => {
  const [searchTerm, setSearchTerm] = React.useState('');
  const outpatientPatients = patients.filter(p => p.status === 'Outpatient');

  const filtered = outpatientPatients.filter(p => 
    ...
    <div className="p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500">Scheduled consultations and walk-in patients.</p>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            type="text" 
            placeholder="Search OPD patient/ID..." 
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filtered.map((patient, i) => (
          <motion.div
            ...
            key={patient.id}
            className="group bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer"
          ...
          >
            <div className="p-6">
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs font-bold text-brand">OPEN RECORD</span>
      ...
      {filtered.length === 0 && (
        <div className="py-20 text-center bg-white rounded-3xl border-2 border-dashed border-slate-200">
          ...
          <p className="text-slate-400">No scheduled outpatients for today.</p>
        </div>
      )}
    </div>
  );
